...
class Oposum {
  constructor({ x, y, width, height, velocity = { x: Oposum_X_VELOCITY, y: 0 } },
    turningDistance = 100
  ) {
    this.x = x
    this.y = y
    ...
    this.currentSprite = this.sprites.run//default position
    this.facing = 'right'
    this.hitbox = {
      x: 0,
      y: 0,
  ...
  updateHorizontalPosition(deltaTime) {
    if(Math.abs(this.distanceTravelled) > this.turningDistance){ //to make sure it a positive num
    ...
    this.distanceTravelled += this.velocity.x * deltaTime
  }
